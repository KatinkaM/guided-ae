...
def get_features(name):
    """
    Function to retrieve the output after a layer given by [name].
    The output is added to features which is retrieved in main_loop
    """
...
class Concat(nn.Module):
    """
    Class that concatenates the skip and deeper network together. 
    """
...
def bn(num_features):
    """
    Function that performs batch normalization.
    """
...
def conv(in_f, out_f, kernel_size, stride=1, bias=True, pad='zero', downsample_mode='stride'):
    """
    Function that creates a convolutional network. 
    Padding and downsapling is not used.
    Returns a pointer to a layer in a Sequential. 
    """
    downsampler = None
    padder = None
...
def conv_gab(in_f, out_f, kernel_size, stride=1, bias=True, pad='zero', downsample_mode='stride'):
    """
    Function that creates a convolutional network. 
    Padding and downsapling is not used.
    Returns a pointer to a layer in a Sequential. 
    ...
    #     padder = nn.ReflectionPad2d(to_pad) #Creates some kind of pad
    #     to_pad = 0
    convolver = GaborConv2d(in_f, out_f, (kernel_size,kernel_size))
    layers = filter(lambda x: x is not None, [padder, convolver, downsampler])
    return nn.Sequential(*layers)  # returns a sequence of the layers
...
def skip(
    ...
    last_scale = n_scales - 1  # 3-1 = 2

    # Modules will be added to it in the order they are passed in the constructor. Chains output to input
    model = nn.Sequential()
    model_tmp = model

    input_depth = num_input_channels  # Amount of bands (filter depth)

    for i in range(len(num_channels_down)):  # Iterate trough amount of blocks

        # Creates containter for skip network and the deep network
        deeper = nn.Sequential()
        skip = nn.Sequential()

        if num_channels_skip[i] != 0:  # For every layer deeper and skip are concatenated to model_tmp
        ...
        # For every layer conv(B,128,1) this is added to skip sequence + bn and leakyRelu
        
        deeper.add(conv_gab(input_depth, num_channels_down[i], filter_size_down[i],
