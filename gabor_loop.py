...
residual_root_path = "./normalNet"

torch.autograd.set_detect_anomaly(True)
AUC_list = []
def auc_and_roc_n(gt,det_n):
  detection = det_n.detach().cpu().squeeze().numpy()
  detection = np.transpose(detection)
  #Calculatin AUC score 
  img_reshape = detection.reshape(detection.shape[0]*detection.shape[1],-1)
  AUC = metrics.roc_auc_score(gt, img_reshape)
  return AUC
...
def main(abu):
    ...
    print(abu)
    #***********************Retrieve GT*************************************************
    gt_path = "C:/Users/katin/Documents/NTNU/Semester_10/data/ABU_data/"+ abu
    gt = sio.loadmat(gt_path)['map']#[:100,:100] #When using KPCA I have to do this!!!!
    ...
    net = skip(input_depth, output_depth,
               num_channels_down=channel_structure,
               num_channels_up=channel_structure,
               num_channels_skip=channel_structure,
    ...
    def closure(iter_num, mask_varr, residual_varr):
        ...
        if iter_num != 0 : #if iter_num % 100 == 0 and iter_num != 0: 
            # weighting block
    ...
    for j in range(num_iter):  # iterate 1001 times
        ...
        AUC_list.append(auc_and_roc_n(gt, residual_varr))
        ...
        if j == num_iter-1 or end_iter == True:  # Happens if iterations = 1000 or end it is True
            ...
            code = features["code"].detach().cpu().squeeze().numpy()
            sio.savemat(residual_path, {
                        'detection': residual_np, 'background': mask_np, 'time': end-start, 'code': code, 'auc_scores': AUC_list})
...
if __name__ == "__main__":
    abu_list = ["abu-airport-2","abu-airport-4","abu-beach-2", "abu-beach-3", "abu-urban-1", "abu-urban-5"]
    for i in range(len(abu_list)):
        abu = abu_list[i]
        t = main(abu)
        residual_path = residual_root_path + abu+".mat" 
        result_list.append(calculate_AUC(residual_path, abu))
        time_list.append(t)
    print(result_list)
    print(time_list)
